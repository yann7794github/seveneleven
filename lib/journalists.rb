...
number = $liste.length
	...
	puts "Il y a #{number} dans mon array"
	puts ""

#Combien porte un numero
number = $liste.grep(/[0-9]/).count
	puts "Combien d'handle contiennent un numéro ?"
	puts number
	puts ""

#Combien de Aude
downcase = $liste.map(&:downcase)
aud = downcase.grep(/aude/).count
	puts "Combien d'handle contiennent les 4 lettres du prénom Aude à la suite ?"
	puts "Il y a #{aud} emails"
	puts ""

#Combien debute par une MAJ
beginmaj = $liste.grep(/@[A-Z]/).count
	puts "Combien commencent par une majuscule ?"
	puts "#{beginmaj}"
	puts ""

#Combien ont une MAJ
maj = $liste.grep(/[A-Z]/).count
	puts "Combien contiennent une majuscule ?"
	puts "Il y a #{maj}"
	puts ""

#Combien de _
tiret = $liste.to_s.count("_")
	puts "Combien de _ dans tous les pseudos confondus ?"
	puts "Il y a #{tiret}"
	puts ""

#Tri de la liste A à Z
order = $liste.sort(&:casecmp)
	puts "Trie la liste de handle par ordre alphabétique"
	puts order

#Quels sont les handle les plus courts
length = $liste.sort_by(&:length)
mako = length.take(50)        
	puts "Trie la liste de handle par ordre de longueur"
	puts mako

#Quel est la position de @epenser dans le array
mario = $liste.index('@epenser')
      puts mario
